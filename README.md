...
登録メニューには`ホットキー: Alt+U（倍率を順番に切り替え）`と`ホットキー: Alt+S（シャープ化を順番に切り替え）`という案内項目も表示されます。これらは操作の説明用で、選択しても設定は変わりません。
...
- 旧版のON/OFF形式で保存されたSpineシャープ化設定は、ONを`中`、OFFを`OFF`として引き継ぎます。
