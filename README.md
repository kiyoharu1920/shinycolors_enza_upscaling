# シャニマス Canvas 高解像度化

ブラウザ版「アイドルマスター シャイニーカラーズ」のCanvas描画を高解像度化するユーザースクリプトです。

ゲーム内の論理座標、画面レイアウト、入力座標を維持したまま、PIXI.jsのCanvas描画解像度を調整します。

## 主な機能

- Canvas描画倍率を`1x`から`4x`まで変更
- 画面サイズ、端末のピクセル密度、GPU上限から倍率を決める「自動」モード
- TampermonkeyとStayの登録メニューに同じユーザー向け設定項目を表示
- PCでは`Alt + U`で即時切り替え
- 選択した設定を保存し、次回読み込み時に復元
- Rendererの再生成、Canvasサイズ変更、画面回転、WebGL context復帰時に再適用

## 設定一覧

| 設定 | 選択肢 | 初期値 | PCホットキー |
| --- | --- | --- | --- |
| Canvas描画倍率 | `自動` / `1x` / `1.5x` / `2x` / `3x` / `4x` | `2x` | `Alt + U` |

ユーザー向け設定はTampermonkeyまたはStayの登録メニューから変更できます。現在選択されているCanvas倍率には`✓`が表示されます。

登録メニューには`ホットキー: Alt+U（倍率を順番に切り替え）`という案内項目も表示されます。これは操作の説明用で、選択しても設定は変わりません。

### Canvas描画倍率

`Alt + U`を押すたびに次の順番で切り替わります。

`自動 → 1x → 1.5x → 2x → 3x → 4x → 自動`

描画倍率を適用するときは、次の値を同期します。

- `renderer.resolution`
- `rootRenderTarget.resolution`
- `plugins.interaction.resolution`
- Canvasのbacking storeサイズ

CanvasのCSS表示サイズとゲームの論理サイズは変更しません。

手動で選択した倍率もGPUの描画サイズ上限を超える場合は制限され、実際の倍率が選択値より低くなることがあります。

## インストール

### Tampermonkey

1. Tampermonkeyへ`shiny_colors_upscaler.js`をインストールします。
2. `https://shinycolors.enza.fun/`を開きます。
3. Tampermonkeyアイコンから本スクリプトのメニューを開き、設定を選択します。

PCではメニューに加え、`Alt + U`を使用できます。

### Stay（iPhone / iPad）

1. iOS/iPadOSの「設定」から`Safari > 拡張機能 > Stay`を開き、Stayを有効にします。
2. StayにすべてのWebサイトへのアクセスを許可します。
3. Stayへ`shiny_colors_upscaler.js`を「スクリプトURL」「直接編集」「ローカルファイル」などから追加します。
4. Stayのライブラリ（資料庫）で本スクリプトを有効にします。
5. `https://shinycolors.enza.fun/`を開き、SafariのStay拡張ポップアップを開きます。
6. マッチしたスクリプトの登録メニューから、Canvas倍率を変更します。

iPhone/iPadではキーボードショートカットを前提とせず、Stayの登録メニューを使用してください。

### 実行タイミングを上書きしないでください

本スクリプトは`@run-at document-start`で実行されることを前提としています。ゲームは起動時に一度だけ`window.ezg`へ代入し、直後に`null`へ戻すため、この瞬間より後に実行されるとゲーム参照を取得できず、倍率が一切適用されません。

Userscript Managerには、スクリプト側の`@run-at`をユーザー設定で上書きする機能があります。次の設定は変更しないでください。

- **Stay**: スクリプト設定の「override meta（メタデータの上書き）」にある`run-at`。`document-end`へ変更すると倍率が適用されなくなります（iPhoneで実際に発生した事例）。
- **Tampermonkey**: ダッシュボードでスクリプトを開いた先の「設定」タブにある実行タイミング（`Run at`）の項目。スクリプト側の値（`document-start`）のままにしてください。

上書きしてしまっている場合、登録メニューの「診断情報を表示」で`取得経路: 未取得`と表示されます。`run-at`を`document-start`へ戻すと解消します。

## 設定の反映と保存

- ホットキーの変更は即時反映され、現在状態が画面左下へ短時間表示されます。
- Tampermonkey/Stayの登録メニューから変更した場合は、設定保存後にページが再読み込みされます。
- GMストレージが利用できる場合は`GM_getValue / GM_setValue`を優先し、利用できない環境では`localStorage`を使用します。
- 旧版のCanvas倍率設定も可能な範囲で自動移行します。一覧にない倍率が保存されていた場合は、最も近い倍率へ読み替えます。

## 注意事項

- ゲーム内のすべての画面でアップスケールが正しく処理されるとは限りません。
- 元画像やSpineテクスチャ自体の解像度は変わらないため、元素材にない細部は復元できません。
- 倍率を上げるほどGPU負荷、GPUメモリ使用量、発熱、消費電力が増えます。
- 動作が重い、表示が乱れる、画面が黒くなる場合は、Canvas倍率を`1x`か`2x`へ下げてください。
- Userscript Manager側で`run-at`を`document-start`以外へ上書きすると動作しません。詳細は「実行タイミングを上書きしないでください」を参照してください。
- ゲーム側の更新により、動作しなくなる可能性があります。
- 本スクリプトは非公式です。株式会社バンダイナムコエンターテインメントおよび関係各社とは関係ありません。
