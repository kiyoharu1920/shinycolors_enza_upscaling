...
ユーザー向け設定はTampermonkeyまたはStayの登録メニューから変更できます。Canvas倍率の現在値には`✓`が表示され、Spineシャープ化は`ON / OFF`で表示されます。
...
手動で選択した倍率もGPUの描画サイズ上限を超える場合は制限され、実際の倍率が選択値より低くなることがあります。選択値と実倍率は後述の診断APIで確認できます。
...
- 初期値は`ON`で、変更した状態は次回読み込み時に復元されます。
...
- Stay APIとの互換性は公式ソースと自動テストで確認していますが、iPhone/iPad + Safari実機での動作は未検証です。実際の動作は端末、Safari、GPU性能の影響を受けます。
